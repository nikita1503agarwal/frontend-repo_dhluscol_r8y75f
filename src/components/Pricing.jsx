    ...
    <section id="pricing" className="relative py-24">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-14 grid md:grid-cols-3 gap-6">
          {tiers.map((t, i) => (
            <motion.div key={t.name} initial={{opacity:0, y:12}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:.5, delay:i*0.05}} className={`relative rounded-2xl border border-white/10 bg-white/5 p-6 ${t.name==='Pro' ? 'ring-2 ring-blue-400/60' : ''}`}>
              ...
              <a href="#" className="mt-6 inline-flex justify-center w-full px-4 py-2 rounded-xl bg-white text-slate-900 font-medium hover:scale-[1.01] transition">Choose {t.name}</a>
            </motion.div>
          ))}
        </div>
      </div>
